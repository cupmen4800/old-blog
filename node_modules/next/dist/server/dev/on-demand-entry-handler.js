...
function onDemandEntryHandler({ maxInactiveAge , multiCompiler , nextConfig , pagesBufferLength , pagesDir , viewsDir , watcher  }) {
    ...
    return {
        async ensurePage (page, clientOnly) {
            const pagePathData = await findPagePathData(pagesDir, page, nextConfig.pageExtensions, viewsDir);
 ...
 */ async function findPagePathData(pagesDir, page, extensions, viewsDir) {
    ...
    if (pagePath !== null) {
        const pageUrl = (0, _ensureLeadingSlash).ensureLeadingSlash((0, _removePagePathTail).removePagePathTail((0, _normalizePathSep).normalizePathSep(pagePath), extensions, !isView));
        const bundleFile = (0, _normalizePagePath).normalizePagePath(pageUrl);
        const bundlePath = _path.posix.join(isView ? 'views' : 'pages', bundleFile);
        const absolutePagePath = (0, _path).join(isView ? viewsDir : pagesDir, pagePath);
        return {
            absolutePagePath,
            bundlePath,
