...
class PagesManifestPlugin {
    constructor({ serverless , dev , isEdgeRuntime , rootEnabled  }){
    ...
    createAssets(compilation, assets) {
        ...
        for (const entrypoint of entrypoints.values()){
            const pagePath = (0, _getRouteFromEntrypoint).default(entrypoint.name, this.rootEnabled);
            if (!pagePath) {
                continue;
            }
        ...
        if (this.rootEnabled) {
            assets[`${!this.dev && !this.isEdgeRuntime ? '../' : ''}` + _constants.VIEW_PATHS_MANIFEST] = new _webpack.sources.RawSource(JSON.stringify({
                ...edgeServerRootPaths,
                ...nodeServerRootPaths
            }, null, 2));
        }
