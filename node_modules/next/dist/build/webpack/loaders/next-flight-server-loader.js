...
async function transformSource(source) {
    ...
    const isServerExt = _utils.serverComponentRegex.test(resourcePath);
...
async function parseModuleInfo({ resourcePath , source , isClientCompilation , resolver  }) {
    ...
    function addClientImport(path) {
        if (_utils.serverComponentRegex.test(path) || hasFlightLoader(path, 'server')) {
            // If it's a server component, we recursively import its dependencies.
            imports.push(path);
        } else if (_utils.clientComponentRegex.test(path)) {
            // Client component.
            imports.push(path);
        } else {
            // Shared component.
        ...
        switch(node.type){
            case 'ImportDeclaration':
                ...
                if (!isClientCompilation) {
                    ...
                    if (_utils.serverComponentRegex.test(importSource)) {
                        continue;
                    }
                    const importDeclarations = source.substring(lastIndex, node.source.span.start - beginPos);
                    if (_utils.clientComponentRegex.test(importSource)) {
