...
const Home:NextPage<{allPostsData: Array<postDataResult>}> = ({allPostsData}: {allPostsData: Array<postDataResult>}) => {
  ...
  const filter = allPostsData.filter(janle => janle.data.janle ==="programming").filter(x => x.data.category === "react")
  ...
  return (
    ...
    <Head>
      ...
      <meta property="og:url" content={`${process.env.NEXT_PUBLIC_SITEDOMAIN}/programming/react`} />
