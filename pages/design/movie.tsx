...
const Home:NextPage<{allPostsData: Array<postDataResult>}> = ({allPostsData}: {allPostsData: Array<postDataResult>}) => {
  ...
  const filter = allPostsData.filter(janle => janle.data.janle ==="design").filter(x => x.data.category === "movie")
  ...
  return (
    <>
    <Head>
      <title>{process.env.NEXT_PUBLIC_SITETITLE} - Movie</title>
      ...
      <meta property="og:url" content={`${process.env.NEXT_PUBLIC_SITEDOMAIN}/design/movie`} />
      <meta property="og:type" content={process.env.NEXT_PUBLIC_INDEXTYPE}/>
      <meta property="og:title" content={`${process.env.NEXT_PUBLIC_SITETITLE} - Movie`}/>
      <meta property="og:description" content={process.env.NEXT_PUBLIC_INDEXDESCRIPTION}/>
    </Head>
    <div className={`${Articles_styles.article_list}`}>
      <h2 className={Articles_styles.home_h2}>Movie</h2>
      {filter_Article}
    </div>
    <ReactPaginate
            previousLabel={'<'}
